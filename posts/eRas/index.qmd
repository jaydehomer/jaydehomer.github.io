...
Setting album colors (courtesy of my best RA Dr. Gabrielle Pfund)
...
library(plotly)
p <- swifty |> 
  ggplot(aes(x = fct_reorder(album_name_og, album_release_year), y = danceability, 
             color = album_name)) + 
  geom_violin(alpha = .2) +
  geom_point() + 
  ylim(c(.25, 1)) +
...
swifty |> 
  ...
  group_by(track_name, isTV, album_name) |> 
...
Everything will be alright if we just keep dancin' like we're 22!
